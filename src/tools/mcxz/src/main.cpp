                   ...
                   MapStrVal &constConfig)
{
    ...
    tfile += "const std::unordered_map<uint16_t, std::string> g_tileNames = {\n";
    for (size_t i = 0; i < tileDefs.size(); ++i)
    {
        Tile &tile = tileDefs[i];
    ...
    for (const auto &[prefix, list] : constConfig)
    {
        tfile += std::format("const std::unordered_map<uint16_t, std::string> g_{0}Names = {{\n", prefix);
